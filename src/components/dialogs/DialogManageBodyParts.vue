<script setup lang="ts">
    ...
    const bodyParts = ref(['肩', '胸', '腿', '核心'])
    const newBodyPart = ref('')

    function addBodyPart() {
        const val = newBodyPart.value.trim()
        if (!val || bodyParts.value.includes(val)) return
        bodyParts.value.push(val)
        newBodyPart.value = ''
    }
    ...
    >
        <div class="body-parts-dialog">
            ...
            <div class="body-parts-dialog-body">
                ...
                <!-- 現有部位列表 -->
                <ul class="parts-list">
                    <li v-for="(part, i) in bodyParts" :key="part" class="part-item">
                        <span class="text-sm text-gray-150 font-medium">{{ part }}</span>
                        <button class="delete-btn" @click="handleDelete(i)">
